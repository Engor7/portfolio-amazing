...
export const fragmentShader = `
  ...
  vec3 sampleScene(vec2 screenUV, float hoverIntensity) {
  ...
  void main() {
    ...
    // Compute hover intensity once from center UV
    float distortion = 1.0 - 0.08 * radius * radius;
    vec2 distortedUV = screenUV * distortion;
    vec2 aspectRatio = vec2(uResolution.x / uResolution.y, 1.0);
    vec2 worldCoord = distortedUV * aspectRatio;
    ...
    vec2 mouseWorldCoord = mouseScreenUV * mouseDistortion * aspectRatio;
    mouseWorldCoord *= uZoom;
    mouseWorldCoord += uOffset;
    vec2 mouseCellId = floor(mouseWorldCoord / uCellSize);

    float cellDistance = length(cellId + 0.5 - (mouseCellId + 0.5));
    float hoverIntensity = (1.0 - smoothstep(0.4, 0.7, cellDistance)) * step(0.0, uMousePos.x);

    // Chromatic aberration — prism fringing at edges
    float caAmount = smoothstep(0.3, 1.5, radius) * 0.02;
    vec2 caDir = screenUV / max(radius, 0.001) * caAmount;

    float r = sampleScene(screenUV + caDir, hoverIntensity).r;
    float g = sampleScene(screenUV, hoverIntensity).g;
    float b = sampleScene(screenUV - caDir, hoverIntensity).b;

    float fade = 1.0 - smoothstep(1.2, 1.8, radius);
    gl_FragColor = vec4(vec3(r, g, b) * fade, 1.0);
  }
`;
